...
#counting number of training sets provided
    ...
    for j in range(num_training_sets):
        ...
        for i in range(8):
            ...
            img_n, lb_n = img.astype(np.uint8), lb.astype(np.uint8) #augmentations 1-8
            inv_img_n = np.flip(img, 0).astype(np.uint8) #augmentations 9-16 
            inv_lb_n = np.flip(lb, 0).astype(np.uint8) #augmentations 9-16 
            del img, lb
            img_result, lb_result = addtl_augs(strength, img_n, lb_n, i) #apply secondary augmentations
            del img_n, lb_n  
            img_result_r,  lb_result_r = third_augs(third_str, img_result, lb_result,  i) #apply tertiary augmentations
            ...
            # v9-16
            inv_img_result, inv_lb_result = addtl_augs(strength, inv_img_n, inv_lb_n, i+8)
            del inv_img_n, inv_lb_n
            inv_img_result_r,  inv_lb_result_r = third_augs(third_str, inv_img_result, inv_lb_result,  i+8)
            ...
            print ('Saving: ', filename)
            hdf5_file = h5py.File(filename, mode='w')
            inv_img_result_f, inv_lb_result_f = dim_convert(inv_img_result_f, inv_lb_result_f)
