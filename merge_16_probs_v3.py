...
def merge_16_probs_v3(folder):
    ...
    vfolderlist = [dir_name for dir_name in os.listdir(os.path.join(folder, '')) if os.path.isdir(os.path.join(folder, dir_name)) and dir_name.startswith('v')]
    ...
    for  fff in range(2, all_files_length-2): #predictions start with 0; Ignore 0&1 and last two, since they are z-padding 
      
        loadfile = filebasename + str(fff) + '.h5'
        print('Merging 16 variations of file ', filebasename, ' ... number ', str(fff-1), ' of ', str(all_files_length-3))
        image = []
    ...
    #Variations 9-16 are inverse organized    
        loadfile_revert = filebasename + str(all_files_length - (fff+1)) + '.h5'
        ...
        output_filename = os.path.join(folder , '%s_%04d.png' % (filebasename,(fff-2)))
        #delete(filename);
        print('write: ', output_filename)
        try:
            skimage.io.imsave(output_filename, skimage.img_as_ubyte(image), as_grey=True)
        except:
