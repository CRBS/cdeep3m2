...
-----------------------------------------------------------------------------"""
    ...
    if ext:
        if '.h5' in ext:
            ...
            imgstack = [np.stack((h5file[key].value,), axis=-1) for key in h5file.keys()]
        elif '.tif' in ext:
        ...
        if tif_list_len+png_list_len == 0:
            ...
            imgstack = [np.stack((np.array(skimage.io.imread(os.path.join(img_path,filename))),), axis=-1) for filename in file_list]
