...
for file_name in os.listdir(h5_folder):
    if file_name.endswith('h5'):
        ...
        for i in range(4):
            for j in range(4):
                if (flag):
                    a = np.concatenate([a, X_data[:, i*256:(i+1)*256, j*256:(j+1)*256, :]], axis=0)
                    b = np.concatenate([b, Y_label[:, i*256:(i+1)*256, j*256:(j+1)*256, :]], axis=0)
                else:
                    a = X_data[:, i*256:(i+1)*256, j*256:(j+1)*256, :]
                    b = Y_label[:, i*256:(i+1)*256, j*256:(j+1)*256, :]
                    flag=1
