...
def imageimporter_large(img_path, area, z_stack, outfolder):
    ...
    if ext:
        if '.h5' in ext:
            ...
            imgstack = imgstack[area[0]:area[1], area[2]:area[3], z_stack[0]:z_stack[1]]
            print ('Processed size:', imgstack.shape)
        elif '.tif' in ext:
            im = pilimage.open(img_path)
            image_array = np.array(im)
            imgstack = image_array[area[0]:area[1], area[2]:area[3], z_stack[0]:z_stack[1]]
        ...
        if tif_list_len+png_list_len == 0:
            ...
            with open(tempmat_outfile, 'w') as f:
                for fl in range(z_stack[0], z_stack[1]):
                    f.write(os.path.join(tempdir, file_list[fl][:-3] + 'png') + '\n')

            crop_png(tempmat_infile, tempmat_outfile, area[0], area[1], area[2], area[3])

            print('Reading images')
            imgstack = np.array([skimage.io.imread(os.path.join(tempdir, file_list[i][:-3] + 'png'),as_gray=True) for i in range(z_stack[0], z_stack[1])])    
    ...
    if area[0]==0 and imgstack.shape[1]<=1012: #first in y
        imgstack = np.concatenate((np.flipud(imgstack[:,1:13,:]), imgstack), axis=1)
    if area[2]==0 and imgstack.shape[2]<=1012: #then in x
        imgstack = np.concatenate((np.fliplr(imgstack[:,:,1:13]), imgstack), axis=2)
    ...
    if x_size<1024:
        ...
        imgstack = np.concatenate((imgstack, np.flipud(imgstack[:, x_size-max_padsize-1:x_size-1,:])), axis=1)

    if y_size<1024:
        max_padsize = 1024 - y_size
        max_padsize = min(max_padsize,12)
        imgstack = np.concatenate((imgstack, np.fliplr(imgstack[:,:,y_size-max_padsize-1:y_size-1])), axis=2)
    ...
    if x_size<1024 or y_size<1024:
        temp_img = np.zeros((imgstack.shape[0], 1024, 1024), dtype=imgstack.dtype)
