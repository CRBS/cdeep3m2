...
def ensemble(sys_argv):
    ...
    def average_img(x):
        ...
        t0 = time()    
        temp = cv2.imread(os.path.join(infolders[0],filelist1[x]), cv2.IMREAD_UNCHANGED)
        # img[:,:,0]
        for n in range(1, len(infolders)):
            temp = np.dstack((temp, cv2.imread(os.path.join(infolders[n],filelist1[x]), cv2.IMREAD_UNCHANGED)))
            print(time()-t0)
            print(temp.shape)    
        ...
        try:
            skimage.io.imsave(os.path.join(outfolder,filelist1[x]), arr, as_grey=True)
        except:
    ...
    Parallel(n_jobs=p_tasks)(delayed(average_img)(i) for i in range(0, len(filelist1)))
