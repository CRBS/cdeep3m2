...
def TV_Chambolle(images, factor):
    augmented_images = []
    for i in range (len(images)):
        sigma_est = estimate_sigma(images[i], multichannel=True, average_sigmas=True)/100
...
def HistogramEqualization(images, factor):
    augmented_images = []
    for i in range (len(images)):
        image_histogram, bins = np.histogram(images[i].flatten(), factor, density=True)
        cdf = image_histogram.cumsum() # cumulative distribution function
        cdf = (factor-1) * cdf / cdf[-1] # normalize
        image_equalized = np.interp(images[i].flatten(), bins[:-1], cdf).reshape(images[i].shape).astype(np.uint8)
...
def Skew(images, labels, factor):
    ...
    for i in range (len(images)):
        imagem = Image.fromarray(images[i][:,:,0], 'L')
        image_mod = np.array(imagem.transform(imagem.size,Image.PERSPECTIVE,perspective_skew_coefficients_matrix, resample=Image.BICUBIC))
        augmented_images.append(np.expand_dims(image_mod, axis=2))
        labelm = Image.fromarray(labels[i][:,:,0], 'L')
        label_mod = np.array(labelm.transform(labelm.size,Image.PERSPECTIVE,perspective_skew_coefficients_matrix, resample=Image.BICUBIC))
...
def ElasticDistortion(images, labels,  sigma):
    ...
    linfit_0 = interp1d([1,len(images)], np.vstack([indices[0][:,0], last_indices[0][:,0]]), axis=0)
    linfit_1 = interp1d([1,len(images)], np.vstack([indices[1][:,0], last_indices[1][:,0]]), axis=0)
   
    i = 1
    for j in range (len(images)):
        indices_n = (np.expand_dims(linfit_0(i), axis =1), np.expand_dims(linfit_1(i), axis =1))
        distorted_image = map_coordinates(images[j][:,:,0], indices_n, order=1, mode='reflect')
        reshaped_img = distorted_image.reshape(images[j][:,:,0].shape)
        augmented_images.append(np.expand_dims(reshaped_img, axis=2))
        distorted_lbl = map_coordinates(labels[i-1][:,:,0], indices_n, order=1, mode='reflect')
...
def Resize(images, labels, scale, dir):
    ...
    if dir == 0: 
        ...
        if (min_x <= 325 or min_y <=325):
            augmented_images, augmented_labels = check_img_dims(np.asarray(augmented_images), np.asarray(augmented_labels), 325)
