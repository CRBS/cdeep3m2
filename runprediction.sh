...
positional arguments:
  ...
  images               Directory of images to process or a single image file to process
...
if [ $? != 0 ] ; then
    fatal_error "$out_dir" "ERROR unable to copy $out_dir/augimages/de_augmentation_info.json to $out_dir" 8
fi
